...
### Authenticator processing steps
 ...
 1. If `genKey` is present:
    ...
    1. Use `salt`, `up`, `uv`, `be` and a per-credential authenticator secret as
        the seeds to deterministically generate a new signing key pair with
        private key `p` and public key `P`.

    1. Let `P_enc` be `P` encoded in COSE_Key format.

    1. Let `keyHandle` be an authenticator-specific encoding of `alg`, `salt`,
         `up`, `uv` and `be`, which the authenticator can later use to derive
         the same key pair `p, P`. The encoding SHOULD include integrity
         protection to ensure that a given `keyHandle` is valid for a particular
         authenticator.
        ...
        1. Let `keyHandleParams = [alg, salt, up, uv, be]` as CBOR.
 ...
 1. If `sign` is present:
    ...
    1. Decode the authenticator-specific encoding of `keyHandle` to extract the
        ...
        1. Parse `[alg, salt, up, uv, be] = keyHandleParams` as CBOR.
    ...
    1. Use `salt`, `up`, `uv`, `be` and a per-credential authenticator secret as
        the seeds to deterministically regenerate the signing key pair with
        private key `p` and public key `P`.
